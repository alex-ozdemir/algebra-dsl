...
use khwarizmi::{
    AlgebraDSLError as Error, ErrorVariant as Variant, Expression, Indexable, KhwarizmiOutput,
...
impl Cmd {
    pub fn execute(self, last: Option<&Math>, history: &Vec<Math>) -> Result<Return, Error> {
        match (self, last) {
            ...
            (Cmd::Factor(index, expr), Some(old_expr)) => Ok(Return::Math(
                old_expr.clone().factor(&index, expr.as_expr(last)?)?,
            )),
            ...
            (Cmd::Output(math_idxs_to_output), _) => {
                ...
                for idx in math_idxs_to_output {
                    latex_writer
                        .add_math(history.get(idx).ok_or_else(|| invalid_hist_idx_err(idx))?)
                        .map_err(|_| internal_err())
                        .unwrap();
                }
                Ok(Return::LaTeXBlock(
                    latex_writer.finish_str().map_err(|_| internal_err())?,
                ))
            }
            (Cmd::Recover(idx), e) => {
            ...
            (Cmd::Flip, Some(_)) => Err(Error::new(
                Variant::NeedsEquation,
            ...
            (Cmd::Distribute(indices, last_index), Some(old_expr)) => {
                Ok(Return::Math(match indices.len() {
                    ...
                    _ => old_expr.clone().distribute_many(
                        &old_expr.sibling_indices(indices.as_slice())?,
                        &last_index,
                    )?,
                }))
            }
            ...
            (c, None) => Err(Error::new(
                Variant::NeedsExpression,
        ...
        if s.starts_with("cmd@") {
            ...
            if s.starts_with("make") {
                ...
                Err(if Math::from_str(s).is_ok() {
                    Error::new(
                        Variant::UnrecognizedCmd,
                        format!(
                            "Did you mean to enter Math? Try using a dollar \
                             sign:\n\t$ {}",
                            s
                        ),
                    )
