...
named!(
    identifier<String>,
    preceded!(sp, map!(map_res!(alpha, str::from_utf8), String::from))
);

named!(
    control_sequence<String>,
    do_parse!(sp >> char!('\\') >> cs: identifier >> (cs))
);
...
named!(
    escaped<String>,
    fold_many0!(none_of!("%"), String::new(), |mut s: String, c: char| {
        s.push(c);
        s
    })
);
...
named!(
    ...
    alt!(
        map!(special_character, Token::Special)
            ...
            | map!(
                delimited!(open_escape, escaped, close_escape),
                Token::Escaped
            )
...
mod tests {
    ...
    fn test_chars() {
        ...
        let expected = IResult::Done(
            &b""[..],
            Token::List(vec![Token::Char('h'), Token::Char('i'), Token::Char('1')]),
        );
        assert_expected_eq_actual!(expected, tokens(input));
    }
    ...
    fn test_number_sequence() {
        ...
        let expected = IResult::Done(
            &b""[..],
            Token::List(vec![Token::Char('1'), Token::Char('3'), Token::Char('4')]),
        );
        assert_expected_eq_actual!(expected, tokens(input));
    }
